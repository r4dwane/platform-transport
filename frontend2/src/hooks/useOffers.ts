...
export const useOffers = () => {
  ...
  const submitOffer = async (payload: CreateOfferPayload) => {
    ...
    try {
      ...
      console.log("OFFER ERROR:", JSON.stringify(e?.response?.data));
      setError(e?.response?.data?.detail ?? "Erreur lors de la soumission.");
      throw e;
    }
  };
 
  const acceptOffer = async (offerId: string, methode: MethodePaiement = MethodePaiement.CASH) => {
  setError(null);
  try {
    const { data } = await offersService.accept(offerId, methode);
    return data.trajet_id as string;
  } catch (e: any) {
    console.log("ACCEPT OFFER ERROR:", JSON.stringify(e?.response?.data));
